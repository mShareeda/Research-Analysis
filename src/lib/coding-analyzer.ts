...
export function classifyLifeCycle(publishedAt: Date | undefined | null): string | undefined {
...
const systemPrompt =
  ...
  "Base every coding decision strictly on the article itself — the headline and body together are the context unit. Do not infer or assume anything not explicitly stated.\n\n" +
  "Field definitions:\n" +
  `- site: which outlet published this (${SITE_OPTIONS.slice(0, -1).join(", ")}). If it is not one of those, pick "Other" and put the real outlet name in siteOther.\n` +
  `- articleType: ${ARTICLE_TYPE_OPTIONS.join(" / ")} — the journalistic form of the piece.\n` +
  ...
  "- bahrainCentrality: Peripheral (Bahrain is mentioned only as the race location), Moderate (Bahrain appears in several parts but is not the main focus), or Central (Bahrain is a primary focus of the article).\n" +
  "- overallTone: the tone of the entire article.\n" +
  "- toneTowardBahrain: evaluate only how Bahrain itself is portrayed — not the race, teams, or drivers. A race report that is upbeat about a driver's win but treats Bahrain as a neutral venue should score toneTowardBahrain as Neutral even if overallTone is Positive.\n" +
  `- dominantContext: the single most dominant context — ${CONTEXT_OPTIONS.join(", ")}.\n` +
  `- dominantNewsFrame: the single dominant frame — ${NEWS_FRAME_OPTIONS.slice(0, -1).join(", ")}, or "Other" with newsFrameOther specifying it.\n` +
  `- dominantImage: the strongest image of Bahrain conveyed — ${IMAGE_OPTIONS.slice(0, -1).join(", ")}, or "Other" with imageOther specifying it.\n` +
  "- notes: brief additional observations that may assist interpretation, or omit if none. Write it in BOTH English and Arabic — natural, fluent text in each language, faithful to the same meaning, not a robotic literal translation.";

const jsonInstruction =
  "Return only valid JSON with exactly these keys: site, siteOther, articleType, lifeCycle, bahrainCentrality, overallTone, toneTowardBahrain, dominantContext, dominantNewsFrame, newsFrameOther, dominantImage, imageOther, notes. " +
  "siteOther/newsFrameOther/imageOther are plain strings, omit or leave empty when not applicable. Every enum field must be exactly one of the listed values, verbatim, in English. " +
  'notes must be a bilingual object shaped like {"en": <English notes>, "ar": <Arabic notes>} — never a bare string. Omit notes entirely, or use empty strings for both languages, if there is nothing to add.';
...
// notes must be {en, ar}. If the model ignored that and returned a bare string, duplicate
// it into both languages rather than failing validation.
function normalizeNotes(value: unknown): { en: string; ar: string } | undefined {
...
export function normalizeCodingJson(value: unknown) {
  ...
  return {
    site: matchEnum(report.site, SITE_OPTIONS, "Other"),
    siteOther: stringOrUndefined(report.siteOther),
    ...
    overallTone: matchEnum(report.overallTone, TONE_OPTIONS, "Neutral"),
    toneTowardBahrain: matchEnum(report.toneTowardBahrain, TONE_OPTIONS, "Neutral"),
    ...
    notes: normalizeNotes(report.notes),
  };
}
